...
def main():
  ...
  for trstr,a,b,c,d,e, in zip(TracerList,IniList,MassList,WatList,MassCVList,WatTr1CV):
    filename1 = ('results/iniProfiles/CNT%sini%s' % (run,trstr))
    mpt.dumpFiles(filename1,a)
    print(filename1)
    
    filename2 = ('results/massShelf/CNT%smass%s' % (run,trstr))
    mpt.dumpFiles(filename2,b)
    print(filename2)
  
    filename3 = ('results/volumeHCWShelf/CNT%swat30%s' % (run,trstr) )
    mpt.dumpFiles(filename3,c)
    print(filename3)
  
    filename4 = ('results/massCV/CNT%smass%s' % (run,trstr))
    mpt.dumpFiles(filename4,d)
    print(filename4)
  
    filename5 =  ('results/volumeHCWCV/CNT%swat30%s' % (run,trstr))
