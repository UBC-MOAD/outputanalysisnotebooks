...
def main():
  ...
  Tr =  rout.getField(Ptracers1,trac)
  target = Tr[0,30,50,180] # threshold concentration
  
  #get masks
  MaskHC_CS1 = mpt.maskHC(target,Tr,CS1[0],CS1[1],CS1[2],CS1[3],CS1[4],CS1[5],CS1[6],CS1[7])# mask cells along slice depending on their tracer concentration
  MaskHC_CS2 = mpt.maskHC(target,Tr,CS2[0],CS2[1],CS2[2],CS2[3],CS2[4],CS2[5],CS2[6],CS2[7])# mask cells along slice depending on their tracer concentration
  MaskHC_CS3 = mpt.maskHC(target,Tr,CS3[0],CS3[1],CS3[2],CS3[3],CS3[4],CS3[5],CS3[6],CS3[7])# mask cells along slice depending on their tracer concentration
  MaskHC_CS4 = mpt.maskHC(target,Tr,CS4[0],CS4[1],CS4[2],CS4[3],CS4[4],CS4[5],CS4[6],CS4[7])# mask cells along slice depending on their tracer concentration
  MaskHC_CS5 = mpt.maskHC(target,Tr,CS5[0],CS1[1],CS1[2],CS1[3],CS1[4],CS1[5],CS1[6],CS1[7])# mask cells along slice depending on their tracer concentration
  MaskHC_CS3sb = mpt.maskHC(target,Tr,CS3sb[0],CS3sb[1],CS3sb[2],CS3sb[3],CS3sb[4],CS3sb[5],CS3sb[6],CS3sb[7])# mask cells along slice depending on their tracer concentration
  MaskHC_AS1 = mpt.maskHC(target,Tr,AS1[0],AS1[1],AS1[2],AS1[3],AS1[4],AS1[5],AS1[6],AS1[7])# mask cells along slice depending on their tracer concentration
  MaskHC_AS2 = mpt.maskHC(target,Tr,AS2[0],AS2[1],AS2[2],AS2[3],AS2[4],AS2[5],AS2[6],AS2[7])# mask cells along slice depending on their tracer concentration
  MaskHC_LID1 = mpt.maskHC(target,Tr,LID1[0],LID1[1],LID1[2],LID1[3],LID1[4],LID1[5],LID1[6],LID1[7])# mask cells along slice depending on their tracer concentration
  MaskHC_LID2 = mpt.maskHC(target,Tr,LID2[0],LID2[1],LID2[2],LID2[3],LID2[4],LID2[5],LID2[6],LID2[7])# mask cells along slice depending on their tracer concentration
  ...
  #add up
  V_CS1 = np.sum(np.sum(np.ma.masked_array(V_CS1a*V_CS1area,mask=MaskHC_CS1),axis=1),axis=1)
  V_CS2 = np.sum(np.sum(np.ma.masked_array(V_CS2a*V_CS2area,mask=MaskHC_CS2),axis=1),axis=1)
  V_CS3 = np.sum(np.sum(np.ma.masked_array(V_CS3a*V_CS3area,mask=MaskHC_CS3),axis=1),axis=1)
  V_CS4 = np.sum(np.sum(np.ma.masked_array(V_CS4a*V_CS4area,mask=MaskHC_CS4),axis=1),axis=1)
  V_CS5 = np.sum(np.sum(np.ma.masked_array(V_CS5a*V_CS5area,mask=MaskHC_CS5),axis=1),axis=1)
  V_CS3sb = np.sum(np.sum(np.ma.masked_array(V_CS3sba*V_CS3sbarea,mask=MaskHC_CS3sb),axis=1),axis=1)
  U_AS1 = np.sum(np.sum(np.ma.masked_array(U_AS1a*U_AS1area,mask=MaskHC_AS1),axis=1),axis=1)
  U_AS2 = np.sum(np.sum(np.ma.masked_array(U_AS2a*U_AS2area,mask=MaskHC_AS2),axis=1),axis=1)
  W_LID1 = np.sum(np.sum(np.ma.masked_array(W_LID1a*W_LID1area,mask=MaskHC_LID1),axis=2),axis=1)
  W_LID2 = np.sum(np.sum(np.ma.masked_array(W_LID2a*W_LID2area,mask=MaskHC_LID2),axis=2),axis=1)
