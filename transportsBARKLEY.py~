...
def main():
  ...
  FluxTR04 = ('%s/%s/FluxTR04Glob.nc' %(expPath, run))
  FluxTR05 = ('%s/%s/FluxTR05Glob.nc' %(expPath, run))
  FluxTR06 = ('%s/%s/FluxTR06Glob.nc' %(expPath, run))
  FluxTR07 = ('%s/%s/FluxTR07Glob.nc' %(expPath, run))
  FluxTR08 = ('%s/%s/FluxTR08Glob.nc' %(expPath, run))
  ...
  fluxfile = [FluxTR01,FluxTR02,FluxTR03,FluxTR04,FluxTR05,FluxTR06,FluxTR07,FluxTR08]
  fluxtr = ['1','2','3','4','5','6','7','8']
  ...
  for f,tr,trstr in zip (fluxfile,fluxtr,TracerList):
    ...
    filename1 = ('results/metricsDataFrames/BAR_CSTRANS_%s%s.csv' % (run,trstr))
    df.to_csv(filename1)
